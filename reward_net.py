...
class RewardNetwork(object):
    ...
    def rank(self,num_to_rank=None):
        if self.rank_by_true_reward:
            k_tau = self.auto_rank(num_to_rank)
        else:
            k_tau = self.manual_rank(num_to_rank)

        return k_tau

    def manual_rank(self, num_to_rank=None):
        ...
        generated_rewards = rewards.detach().cpu().numpy().tolist()

        print('gene rewards:', generated_rewards)

        rank_index = []
        rank_label = []
        rank_input = input('please score trajectories (index:score)')
        for item in rank_input.split(','):
            rank_index.append(int(item.split(':')[0])-1)
            rank_label.append(float(item.split(':')[1])+2)

        rank_traj = []
        for i in range(len(rank_index)):
            rank_traj.append(self.buffer[rank_index[i]])

        _, rank_label_true = self.get_rank(rank_index)
    ...
    def auto_rank(self, num_to_rank=None):
        ...
        if self.sample_method == 'random sample':
            ...
            rank_traj, rank_index = self.state_entropy_sample_buffer(num_to_rank)
        else:
            raise Exception('wrong sample method for reward learning')
        
        rank_label, rank_label_true = self.get_rank(rank_index)

        ########################################################
        if 'entropy' in self.sample_method or 'entropy' in self.make_batch_method:
            states = torch.FloatTensor(np.array([sa[:self.state_dim] for traj in rank_traj for sa in traj])).to(device)
        ...
        if 'entropy' in self.make_batch_method:
            self.update_sample_prob_entropy(self.entropy_alpha, num_to_rank)
        if 'priority' in self.make_batch_method:
    ...
    def update_sample_prob_priority(self, prio_alpha, num_to_rank):
        # bonus = [0 for _ in range(len(self.ranked_labels)-num_to_rank)] + [self.prio_bonus for _ in range(num_to_rank)]
        # priority = self.ranked_labels
        # priority = list(np.add(priority, bonus))
        # priority = list(map(lambda x: x ** prio_alpha, priority))
